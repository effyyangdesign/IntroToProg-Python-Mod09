...
class EmployeeIO:
    ...
    def print_current_list_items(list_of_rows: list):
        ...
        for row in list_of_rows:
            empConstruct = DC.Employee(row[0],row[1],row[2].strip())
            print (empConstruct.to_string())
        print("*******************************************")
        print()  # Add an extra line for looks
        return list_of_rows
    @staticmethod
    def input_employee_data(list_of_rows):
        ...
        try:
            ...
            lstAppend = [str(emp.employee_id),str(emp.first_name),str(emp.last_name)]
            list_of_rows.append(lstAppend)
